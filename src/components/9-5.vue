<template>
    ...
    <p>类似 getter，action 也可通过 this 访问整个 store 实例，并支持完整的类型标注(以及自动补全✨)。不同的是，action 可以是异步的，你可以在它们里面 await 调用任何 API，以及其他 action！</p>
    ...
    <h2>访问其他 store 的 action</h2>
    <p>想要使用另一个 store 的话，那你直接在 action 中调用就好了：</p>
...
const code4 = `import { useAuthStore } from './auth-store'
